...
export class ContextSynthesizer {
     ...
     */
    static synthesize(memories, options = {}) {
     ...
     */
    static extractPatterns(memories, minFrequency) {
    ...
    static extractPhrases(text) {
        ...
        for (const sentence of sentences) {
            ...
            if (trimmed.match(/\b(use|implement|fix|improve|add|create|design|test)\b/i) &&
                trimmed.length > 10 &&
                trimmed.length < 100) {
     ...
     */
    static generateKeyInsights(memories, successRate, averageReward) {
        const insights = [];
     ...
     */
    static generateRecommendations(memories, patterns, successRate) {
     ...
     */
    static generateSummary(memories, patterns, successRate, averageReward) {
        const parts = [];
        // Opening
     ...
     */
    static extractActionableSteps(memories) {
        const steps = [];
        const successfulMemories = memories.filter(m => m.success && (m.reward || 0) >= 0.8);
...
//# sourceMappingURL=ContextSynthesizer.js.map