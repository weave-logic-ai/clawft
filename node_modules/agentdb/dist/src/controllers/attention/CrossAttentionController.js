...
export class CrossAttentionController {
     ...
     */
    addToContext(contextName, entry) {
     ...
     */
    removeFromContext(contextName, id) {
     ...
     */
    async computeCrossAttention(query, contextName, options = {}) {
     ...
     */
    async computeMultiContextAttention(query, contextNames = [], options = {}) {
        const startTime = performance.now();
        ...
        const attended = this.aggregateAttendedOutputs(query, contextResults.map(r => r.attended), Object.values(contextWeights), aggregation);
        const executionTimeMs = performance.now() - startTime;
        return {
            scores: allScores,
            attended,
     ...
     */
    aggregateAttendedOutputs(query, outputs, weights, strategy) {
        if (outputs.length === 0) {
            return [...query];
        }
        const dimension = query.length;
...
//# sourceMappingURL=CrossAttentionController.js.map