...
export class MultiHeadAttentionController {
     ...
     */
    initializeProjections(dimension) {
     ...
     */
    projectVector(vector, headIndex) {
     ...
     */
    async computeMultiHeadAttention(query, options = {}) {
        ...
        const { numHeads = 8, topK = 10, minScore = 0.0, temperature = 1.0, aggregation = 'average' } = config;
        // Handle empty memory
        if (this.memoryStore.size === 0) {
            const emptyHeads = Array(numHeads).fill(null).map((_, i) => ({
                headIndex: i,
                attended: [...query].slice(i * Math.floor(query.length / numHeads), (i + 1) * Math.floor(query.length / numHeads)),
                topScores: []
            }));
            return {
                heads: emptyHeads,
        ...
        for (let h = 0; h < numHeads; h++) {
            const headResult = await this.computeHeadAttention(query, h, topK, minScore, temperature);
     ...
     */
    async computeHeadAttention(query, headIndex, topK, minScore, temperature) {
     ...
     */
    aggregateHeadOutputs(query, heads, strategy) {
        if (heads.length === 0) {
            return [...query];
        }
     ...
     */
    reconstructFromHead(headOutput, headIndex, targetDim) {
...
//# sourceMappingURL=MultiHeadAttentionController.js.map