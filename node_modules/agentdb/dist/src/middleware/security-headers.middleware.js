...
export function configureCORS(options = {}) {
    const { allowedOrigins = ['http://localhost:3000'], allowCredentials = true, maxAge = 86400, // 24 hours
     } = options;
    return (req, res, next) => {
 ...
 */
export function hstsPreload(req, res, next) {
...
export function applySecurity(options = {}) {
    const { cors = true, corsOrigins = ['http://localhost:3000'], hsts = true, csp = true, } = options;
    const middlewares = [];
    // Remove sensitive headers
 ...
 */
export function developmentCORS(req, res, next) {
