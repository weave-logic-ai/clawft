...
export class ReportStore {
    ...
    `).run(result.scenario, result.scenario, result.timestamp.toISOString(), JSON.stringify(result.config), result.config.profile, '2.0.0', // TODO: Get from package.json
        result.duration || 0, result.iterations || 1, 'completed');
        const simulationId = insertResult.lastInsertRowid;
        // Insert metrics
    ...
    async compare(ids) {
        ...
        for (const [metricName, data] of Object.entries(metrics)) {
            ...
            insights.push(`${metricName}: Best run improved by ${improvement}% over worst`);
        }
        return {
     ...
     */
    async getTrends(scenarioId, metric) {
        if (!this.db)
            throw new Error('Database not initialized');
     ...
     */
    async detectRegressions(scenarioId, threshold = 0.1) {
        if (!this.db)
            throw new Error('Database not initialized');
 ...
 */
export async function createReportStore(dbPath = ':memory:') {
