...
export class ConfigManager {
    ...
    loadWithEnv(baseConfig) {
        ...
        if (process.env.AGENTDB_TRAVERSAL_STRATEGY) {
            config.traversal.strategy = process.env.AGENTDB_TRAVERSAL_STRATEGY;
        }
        // Storage overrides
     ...
     */
    loadDefault(profile = 'production') {
        ...
        const userConfig = path.join(process.env.HOME || '', '.agentdb', 'config.json');
        if (fs.existsSync(userConfig)) {
            return this.loadWithEnv(this.loadFromFile(userConfig));
        }
        // 3. Use preset profile
        return this.loadWithEnv(this.loadProfile(profile));
    }
    // --------------------------------------------------------------------------
    // Saving
    // --------------------------------------------------------------------------
