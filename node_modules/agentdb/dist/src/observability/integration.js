 ...
 */
export async function withTelemetry(operationType, tableName, operation) {
    const startTime = Date.now();
    const telemetry = TelemetryManager.getInstance();
    return withSpan(`agentdb.${operationType}`, async (span) => {
        if (span) {
 ...
 */
export async function withBatchTelemetry(operationType, tableName, batchSize, operation) {
 ...
 */
export function recordCacheAccess(key, hit) {
    if (hit) {
 ...
 */
export function recordErrorWithContext(error, operation, context) {
    recordMetric('error', {
