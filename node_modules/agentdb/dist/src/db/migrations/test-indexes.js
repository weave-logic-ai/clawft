...
function createTestDatabase() {
  ...
  `);
    ...
    for (let s = 0; s < sessions; s++) {
        ...
        for (let e = 0; e < episodesPerSession; e++) {
            insertEpisode.run(sessionId, `Task ${e % 10}`, `Input ${e}`, `Output ${e}`, `Critique ${e}`, Math.random(), Math.random() > 0.5 ? 1 : 0, Math.floor(Math.random() * 1000), Math.floor(Math.random() * 5000));
        }
    }
  ...
  `);
    for (let i = 0; i < 500; i++) {
        insertSkill.run(`Skill ${i}`, `Description for skill ${i}`, JSON.stringify({ inputs: {}, outputs: {} }), Math.random(), Math.floor(Math.random() * 100), Math.random(), Math.floor(Math.random() * 500));
    }
  ...
  `);
    ...
    for (let i = 0; i < 1000; i++) {
        insertPattern.run(taskTypes[i % taskTypes.length], `Approach ${i}`, Math.random(), Math.floor(Math.random() * 50), Math.random());
    }
  ...
  `);
    for (let i = 0; i < 2000; i++) {
        insertEdge.run(Math.floor(Math.random() * 5000) + 1, 'episode', Math.floor(Math.random() * 5000) + 1, 'episode', Math.random(), Math.random() * 2 - 1, Math.random(), Math.floor(Math.random() * 100));
    }
 ...
 */
function benchmarkQuery(db, query, params, description) {
...
function runBenchmarks(db) {
    ...
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM episodes WHERE session_id = ? ORDER BY ts DESC LIMIT 10', ['session-0'], 'Episode query: session_id + ORDER BY ts DESC'));
    // Benchmark 2: Episode query with session_id and task
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM episodes WHERE session_id = ? AND task = ? ORDER BY reward DESC', ['session-0', 'Task 5'], 'Episode query: session_id + task + ORDER BY reward'));
    // Benchmark 3: Skills ordered by success_rate and uses
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM skills ORDER BY success_rate DESC, uses DESC LIMIT 10', [], 'Skills query: ORDER BY success_rate DESC, uses DESC'));
    // Benchmark 4: Reasoning patterns by task_type and success_rate
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM reasoning_patterns WHERE task_type = ? ORDER BY success_rate DESC LIMIT 10', ['coding'], 'Patterns query: task_type + ORDER BY success_rate'));
    // Benchmark 5: Causal edges lookup
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM causal_edges WHERE from_memory_id = ? AND to_memory_id = ?', [100, 200], 'Causal edges: from_memory_id + to_memory_id'));
    // Benchmark 6: Causal edges by confidence
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM causal_edges WHERE from_memory_id = ? ORDER BY similarity DESC LIMIT 10', [100], 'Causal edges: from_memory_id + ORDER BY similarity'));
    // Benchmark 7: Episode with task and success
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM episodes WHERE task = ? AND success = 1 ORDER BY reward DESC LIMIT 10', ['Task 5'], 'Episodes: task + success + ORDER BY reward'));
    // Benchmark 8: Skills with success_rate threshold
    benchmarks.push(benchmarkQuery(db, 'SELECT * FROM skills WHERE success_rate >= 0.7 ORDER BY avg_reward DESC LIMIT 10', [], 'Skills: success_rate threshold + ORDER BY avg_reward'));
    return benchmarks;
}
/**
