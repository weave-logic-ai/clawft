 ...
 * - Audit logging
 */
import { hashPassword, verifyPassword, generateApiKey, hashApiKey, verifyApiKey, generateSessionId, } from '../utils/crypto.utils.js';
import { createTokenPair, revokeToken, } from './token.service.js';
/**
 ...
 * Register a new user
 */
export async function registerUser(email, password, role = 'user', permissions = []) {
    // Validate email
 ...
 */
export function logout(sessionId, refreshToken) {
    // Remove session
 ...
 */
export async function createUserApiKey(userId, name, environment = 'live', expiresInDays) {
 ...
 * Revoke API key
 */
export function revokeApiKey(keyId, reason) {
 ...
 */
export async function rotateApiKey(keyId, reason = 'Key rotation') {
    ...
    // Create new key
    const newKeyResult = await createUserApiKey(oldKey.userId, oldKey.name, oldKey.environment);
