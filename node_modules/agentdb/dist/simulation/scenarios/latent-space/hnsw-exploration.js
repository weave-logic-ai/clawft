 ...
 */
export const hnswExplorationScenario = {
    id: 'hnsw-exploration',
    ...
    async run(config) {
            ...
            for (const vectorCount of config.vectorCounts) {
                for (const dim of config.dimensions) {
                    for (const params of config.hnswParams) {
                        ...
                        const searchMetrics = await measureSearchPerformance(index, config.kValues, config.iterations);
                        // Calculate recall
        ...
        return {
            ...
            summary: {
                ...
                vectorCountsT: config.vectorCounts.length,
 ...
 */
async function buildHNSWIndex(backend, vectorCount, dimension, params) {
 ...
 */
async function analyzeGraphTopology(index) {
 ...
 */
async function measureSearchPerformance(index, kValues, iterations) {
    const latencies = [];
 ...
 */
async function calculateRecall(index, kValues) {
    const recalls = [];
...
// Helper functions
function generateRandomVectors(count, dimension) {
...
function calculateAveragePathLength(index) {
    // Simulated calculation
    return Math.log2(index.vectorCount) * 1.2;
}
function calculateClusteringCoefficient(index) {
    // Simulated calculation
    return 0.3 + (index.params.M / 100) * 0.2;
}
function simulateSearchPaths(index, iterations) {
...
function estimateMemoryUsage(index) {
...
function findBestPerformance(results) {
    return results.reduce((best, current) => current.qps > best.qps ? current : best);
}
...
function aggregateGraphMetrics(results) {
    return {
        averageSmallWorldIndex: results.reduce((sum, r) => sum + r.graphMetrics.smallWorldIndex, 0) / results.length,
        averageClusteringCoeff: results.reduce((sum, r) => sum + r.graphMetrics.clusteringCoefficient, 0) / results.length,
    };
}
function aggregateSearchMetrics(results) {
    return {
...
function analyzeParameterImpact(results) {
    return {
...
async function generateGraphVisualizations(results) {
    return {
...
async function generatePerformanceCharts(results) {
    return {
...
//# sourceMappingURL=hnsw-exploration.js.map