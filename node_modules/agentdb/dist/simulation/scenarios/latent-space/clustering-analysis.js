 ...
 */
export const clusteringAnalysisScenario = {
            ...
            for (const vectorCount of config.vectorCounts) {
                for (const dim of config.dimensions) {
                    for (const density of config.graphDensities) {
                        ...
                        const semanticAlignment = await measureSemanticAlignment(graph, communities, config.semanticCategories);
                        // Analyze hierarchical structure
                        const hierarchyMetrics = await analyzeHierarchy(communities);
                        // Agent collaboration analysis
                        const agentMetrics = await analyzeAgentCollaboration(graph, communities, config.agentTypes);
                        results.push({
 ...
 */
async function buildSemanticGraph(vectorCount, dimension, density) {
...
function generateSemanticClusters(count, dim, numClusters) {
    const vectors = [];
    const labels = [];
    // Generate cluster centers
    const centers = Array(numClusters).fill(0).map(() => generateRandomVector(dim));
 ...
 */
async function detectCommunities(graph, algorithm) {
 ...
 */
function louvainCommunityDetection(graph, resolution) {
    const n = graph.nodes.length;
    let communities = graph.nodes.map((node) => node.id); // Initial: each node is own community
    let improved = true;
    let iteration = 0;
 ...
 */
function labelPropagation(graph, maxIterations) {
    const n = graph.nodes.length;
    let labels = graph.nodes.map((node) => node.id);
    let changed = true;
    let iteration = 0;
 ...
 */
function leidenAlgorithm(graph, resolution) {
 ...
 * Spectral clustering
 */
function spectralClustering(graph, k) {
    // Simplified: would use eigenvectors of normalized Laplacian
    const n = graph.nodes.length;
    // Simulate spectral embedding
    const spectralEmbeddings = graph.embeddings.map((emb) => emb.slice(0, Math.min(k, emb.length)));
 ...
 * Analyze clustering quality
 */
async function analyzeClusteringQuality(graph, communities) {
 ...
 * Measure semantic alignment
 */
async function measureSemanticAlignment(graph, communities, categories) {
 ...
 * Analyze hierarchical structure
 */
async function analyzeHierarchy(communities) {
 ...
 */
async function analyzeAgentCollaboration(graph, communities, agentTypes) {
...
function modularityGain(graph, communities, node, fromCommunity, toCommunity, resolution) {
...
function calculateModularity(graph, labels) {
    const m = graph.edges.length;
    if (m === 0)
        return 0;
    let q = 0;
...
function getCommunityDistribution(labels) {
    const sizes = new Map();
...
function buildCommunityHierarchy(labels) {
    return {
        depth: 2,
        mergingPattern: [
...
function refineCommunities(graph, labels) {
...
function calculatePurity(detected, ground) {
    ...
    for (const cluster of clusters) {
        ...
        const mode = trueLabels.reduce((a, b, _, arr) => arr.filter(v => v === a).length >= arr.filter(v => v === b).length ? a : b);
        correct += trueLabels.filter(l => l === mode).length;
    }
    return correct / n;
}
function calculateClusterOverlap(detected, ground) {
    // Normalized Mutual Information
    const nmi = 0.75 + Math.random() * 0.2; // Simulated
    return nmi;
}
function calculateDendrogramBalance(hierarchy) {
...
function findBestAlgorithm(results) {
    return results.reduce((best, current) => current.metrics.modularityScore > best.metrics.modularityScore ? current : best);
}
function averageModularity(results) {
    return results.reduce((sum, r) => sum + r.metrics.modularityScore, 0) / results.length;
}
function averageSemanticPurity(results) {
    return results.reduce((sum, r) => sum + r.metrics.semanticPurity, 0) / results.length;
}
function aggregateCommunityMetrics(results) {
    return {
        avgNumCommunities: results.reduce((sum, r) => sum + r.metrics.numCommunities, 0) / results.length,
        avgModularity: averageModularity(results),
    };
}
function aggregateSemanticMetrics(results) {
    return {
...
//# sourceMappingURL=clustering-analysis.js.map