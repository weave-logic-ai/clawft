 ...
 */
function safeJSONParse<T>(json: string, validator: (obj: unknown) => obj is T): T {
  let parsed: unknown;
  try {
 ...
 */
export function quantize8bit(vector: Float32Array): QuantizedVector {
  // Input validation
 ...
 */
export function quantize4bit(vector: Float32Array): QuantizedVector {
  // Input validation
  validateFloat32Array(vector, 'Input vector');
  validateDimension(vector.length, 'Vector dimension');
 ...
 */
export function dequantize8bit(
 ...
 * @returns Reconstructed Float32Array
 */
export function dequantize4bit(
  quantized: Uint8Array,
  min: number,
  max: number,
...
): { meanError: number; maxError: number; mse: number } {
 ...
 */
export function getQuantizationStats(
  vector: Float32Array,
  type: '8bit' | '4bit'
): QuantizationStats {
  ...
  const compressedBytes =
    type === '8bit' ? quantized.data.length : Math.ceil(vector.length / 2);
...
export class ProductQuantizer {
  ...
  async train(vectors: Float32Array[]): Promise<void> {
    ...
    if (vectors.length < this.config.numCentroids) {
      throw new Error(
        `Need at least ${this.config.numCentroids} vectors to train ${this.config.numCentroids} centroids`
      );
    }
    ...
    for (const vec of vectors) {
      ...
      if (vec.length !== this.config.dimension) {
        throw new Error(
          `Vector dimension (${vec.length}) does not match config (${this.config.dimension})`
        );
      }
    }
    ...
    for (let s = 0; s < this.config.numSubspaces; s++) {
      ...
      const subvectors = vectors.map((v) =>
        v.slice(startDim, startDim + this.subspaceDim)
      );
   ...
   */
  private async kMeans(subvectors: Float32Array[]): Promise<Float32Array[]> {
    ...
    for (let iter = 0; iter < this.config.maxIterations; iter++) {
      ...
      const sums = Array.from(
        { length: k },
        () => new Float32Array(this.subspaceDim)
      );
  ...
  encode(vector: Float32Array): PQEncodedVector {
    ...
    if (vector.length !== this.config.dimension) {
      throw new Error(
        `Vector dimension (${vector.length}) does not match config (${this.config.dimension})`
      );
    }
  ...
  exportCodebooks(): string {
    ...
    return JSON.stringify({
      config: this.config,
      codebooks: this.codebooks.map((subspace) =>
        subspace.map((centroid) => Array.from(centroid))
      ),
    });
  }
  ...
  importCodebooks(json: string): void {
    ...
    function isValidCodebookData(obj: unknown): obj is {
      ...
      if (typeof config.dimension !== 'number' || !Number.isFinite(config.dimension)) return false;
      if (typeof config.numSubspaces !== 'number' || !Number.isFinite(config.numSubspaces)) return false;
      if (typeof config.numCentroids !== 'number' || !Number.isFinite(config.numCentroids)) return false;

      // Validate codebooks is array of arrays
      if (!Array.isArray(data.codebooks)) return false;
      for (const subspace of data.codebooks) {
        if (!Array.isArray(subspace)) return false;
    ...
    this.subspaceDim = this.config.dimension / this.config.numSubspaces;
    this.codebooks = data.codebooks.map((subspace: number[][]) =>
      subspace.map((centroid: number[]) => new Float32Array(centroid))
    );
    this.trained = true;
  }
   ...
   */
  private squaredL2Distance(a: Float32Array, b: Float32Array): number {
    let sum = 0;
...
/**
 * Stored vector entry in the quantized store
 */
interface StoredVector {
  id: string;
  quantized: QuantizedVector | PQEncodedVector;
  metadata?: Record<string, unknown>;
}
...
export class QuantizedVectorStore {
  private config: Required<QuantizedVectorStoreConfig>;
  ...
  insert(
    ...
    if (vector.length !== this.config.dimension) {
      throw new Error(
        `Vector dimension (${vector.length}) does not match store dimension (${this.config.dimension})`
      );
    }
    ...
    let quantized: QuantizedVector | PQEncodedVector;
    ...
    switch (this.config.quantizationType) {
      ...
      case 'product':
        if (!this.productQuantizer?.isTrained()) {
          throw new Error(
            'ProductQuantizer must be trained before inserting vectors'
          );
        }
  ...
  search(query: Float32Array, k: number, threshold?: number): QuantizedSearchResult[] {
    if (query.length !== this.config.dimension) {
      throw new Error(
        `Query dimension (${query.length}) does not match store dimension (${this.config.dimension})`
      );
    }
    ...
    if (
      this.config.quantizationType === 'product' &&
    ...
    for (const stored of this.vectors.values()) {
      const distance = this.computeAsymmetricDistance(
        normalizedQuery,
        stored.quantized,
        distanceTables
      );
  ...
  export(): string {
    const data: {
      config: Required<QuantizedVectorStoreConfig>;
      vectors: Array<{
        id: string;
        quantized: unknown;
        metadata?: Record<string, unknown>;
    ...
    for (const stored of this.vectors.values()) {
      const exportedQuantized =
        this.config.quantizationType === 'product'
          ? {
              codes: Array.from((stored.quantized as PQEncodedVector).codes),
              norm: (stored.quantized as PQEncodedVector).norm,
            }
          : {
              data: Array.from((stored.quantized as QuantizedVector).data),
              min: (stored.quantized as QuantizedVector).min,
              max: (stored.quantized as QuantizedVector).max,
              dimension: (stored.quantized as QuantizedVector).dimension,
              type: (stored.quantized as QuantizedVector).type,
            };
  ...
  import(json: string): void {
    ...
    for (const stored of data.vectors) {
      ...
      this.vectors.set(stored.id, {
        id: stored.id,
        quantized: quantized as QuantizedVector | PQEncodedVector,
        metadata: stored.metadata,
      });
    }
  }
  ...
  private computeAsymmetricDistance(
    query: Float32Array,
    quantized: QuantizedVector | PQEncodedVector,
    distanceTables: Float32Array[] | null
  ): number {
    switch (this.config.quantizationType) {
      case 'scalar8bit':
      case 'scalar4bit': {
        const q = quantized as QuantizedVector;
        const dequantized =
          q.type === '8bit'
        ...
        const normalizedDeq =
          this.config.metric === 'cosine' ? this.normalize(dequantized) : dequantized;

        return this.squaredL2Distance(query, normalizedDeq);
      }
      case 'product': {
        if (distanceTables && this.productQuantizer) {
          return this.productQuantizer.distanceFromTables(
            distanceTables,
            quantized as PQEncodedVector
          );
        }
        if (this.productQuantizer) {
          return this.productQuantizer.asymmetricDistance(
            query,
            quantized as PQEncodedVector
          );
        }
  ...
  private dequantizeStored(
    ...
    if (this.config.quantizationType === 'product') {
      ...
      return this.productQuantizer.decode(quantized as PQEncodedVector);
    }
 ...
 */
export function createScalar8BitStore(
  dimension: number,
 ...
 */
export function createScalar4BitStore(
  dimension: number,
  metric: 'cosine' | 'l2' | 'ip' = 'cosine'
): QuantizedVectorStore {
  return new QuantizedVectorStore({
 ...
 */
export function createProductQuantizedStore(
  dimension: number,
  numSubspaces = 8,
  numCentroids = 256,
  metric: 'cosine' | 'l2' | 'ip' = 'cosine'
