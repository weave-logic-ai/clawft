...
/// The five scored inputs (plus the hard-interrupt flag) feeding
...
/// Score a candidate's urgency for the floor (ADR-062 D4).
///
/// ```text
/// urgency = 0.30·semantic + 0.15·arousal + 0.20·wait + 0.10·crowd + 0.25·readiness
...
pub fn compute_urgency(s: &UrgencySignals) -> f32 {
    ...
    W_SEMANTIC * c(s.semantic_relevance)
