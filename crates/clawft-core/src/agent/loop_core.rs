...
impl<P: Platform> AgentLoop<P> {
    ...
    ///   `{"deferred": true, "reason": ...}` so the model can re-plan
    ///   (v1). With a [`super::defer::DeferInteractor`] (WEFT-331):
    ///   suspends until allow / deny / cancel / timeout; allow falls
    ///   through to sandbox + dispatch.
