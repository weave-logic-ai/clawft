...
/// (seeds the model chip when `llm.models` hasn't landed yet).
pub fn paint(ui: &mut egui::Ui, path: &str, value: &Value, view: &mut ChatView, live: &Arc<Live>) {
    ...
    // WEFT-256: restore a previously-picked model from egui memory
    // (panel reload / re-select within the session), then seed from
    // the sentinel when still empty so the header is never blank.
