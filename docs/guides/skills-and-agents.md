...
| `filesystem` | no | FS allowlist (`readable-paths`, `writable-paths`, `allow-create`, `allow-delete`). **Required semantics for autogenerated skills** — see [Autogen filesystem allowlist](../skills/autogen.md) |
| `autogenerated` | no | `true` when produced by the skill autogen pipeline |
