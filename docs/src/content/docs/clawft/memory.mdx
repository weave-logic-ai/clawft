...
## Vector index bootstrap (WEFT-84)

Long-term memory can be indexed into a vector store (`memory.rvf` next to
`MEMORY.md`) for semantic search via `clawft_core::memory_bootstrap`.

### When the index is rebuilt

| Trigger | Behavior |
|---------|----------|
| Index missing | `bootstrap_memory_index` builds from `MEMORY.md` |
| `MEMORY.md` mtime **newer** than the index | Automatic reindex on bootstrap |
| Index up to date | Bootstrap skips (returns `0`) |
| Explicit CLI | `weft memory reindex` always force-rebuilds |

After editing `MEMORY.md` by hand (or via tools that do not touch the index),
either wait for the next bootstrap path that runs mtime checks, or run:

```bash
weft memory reindex
```

This embeds each section (header- or paragraph-split) with the local hash
embedder and writes `memory.rvf` (plus a `.sidecar.json` companion when the
real RVF backend is active). Deleting the index manually is no longer required.
...
weft memory show
weft memory history
weft memory search "query"
weft memory reindex
weft memory export --agent <id> --output path.json
weft memory import --agent <id> --input path.json
```
