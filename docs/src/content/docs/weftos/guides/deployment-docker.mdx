...
description: Complete Docker deployment guide including image builds, configuration, Docker Compose, CI/CD pipelines, and multi-arch releases.
---

# Docker Deployment

WeftOS ships a minimal Alpine 3.21 Docker image built from a **self-contained
multi-stage Dockerfile** (compiles static musl `weft` from the tag — no
cargo-dist asset download; WEFT-594). Typical compressed size is ~30–40 MB.
Multi-arch (`linux/amd64` + `linux/arm64`) is produced on native GitHub
runners (no QEMU for arm64 Rust). Canonical operator guide:
[docs/deployment/docker.md](https://github.com/weave-logic-ai/weftos/blob/master/docs/deployment/docker.md).
...
### Multi-Arch Docker Release (WEFT-594)

Release images are published by `.github/workflows/release-docker.yml`:

1. Self-contained multi-stage build of `weft` (static musl) on native runners
   — `ubuntu-latest` → `linux/amd64`, `ubuntu-24.04-arm` → `linux/arm64`
   (no QEMU for compilation).
2. Per-platform digests merged with `docker buildx imagetools create` into
   `ghcr.io/weave-logic-ai/weftos:latest` and `:vX.Y.Z`.
3. Post-publish `GET /api/health` smoke (WEFT-550).
...
The runtime is Alpine 3.21 + `ca-certificates` + `tzdata`, runs as non-root
`weft`, and ships a default config enabling the auth-gated API on `:8080`.

Same OCI multi-arch image works with Docker Desktop, OrbStack, and the
Apple container CLI on macOS (pulls the `linux/arm64` layer natively on
Apple Silicon).
...
|---------|------|----------|----------|
| Default (root `Dockerfile`) | `alpine:3.21` | default CLI features (channels+services+delegate+api) | Production gateway / GHCR |
| Kernel-only (`Dockerfile.alpine`) | `alpine:3.21` | `weaver` native | Kernel-only / compose under crates/clawft-kernel |
...
- [Installation](/weftos/getting-started/installation) -- source build instructions
- [Configuration](/weftos/guides/configuration) -- runtime configuration reference
