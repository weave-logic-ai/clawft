...
Both domains share the same underlying graph infrastructure: BLAKE3-hashed entity IDs, confidence-weighted edges, community detection (label propagation by default; optional SASE spectral clustering), and petgraph-backed traversal.
...
Two partition algorithms share the same post-processing pipeline (WEFT-516).

### Label propagation (default)

Each node starts with a unique label. In each iteration, every node adopts the most frequent label among its neighbors (ties broken by smallest label for determinism). Converges when no labels change, with a 50-iteration safety cap. This remains the default for `cluster()` unless the crate is built with `--features sase-cluster`.

### SASE (selectable)

**SASE** (Scalable and Adaptive Spectral Embedding; Liu et al., CIKM 2024, arXiv:2408.05765) is available via `ClusterMethod::Sase` / `cluster_with`. Pipeline:

1. Deterministic Gaussian initial features
2. Adaptive k-order simple graph convolution (`S = D⁻¹A`) until embedding stabilizes
3. Random Fourier Features map into an approximate spectral space
4. k-means on the embeddings (modularity-guided k on small graphs; √n heuristic on large ones)

Enable Cargo feature `sase-cluster` to make SASE the default for `cluster()` once parity benchmarks justify the migration. Until then, label-propagation stays default; call `cluster_with(kg, ClusterMethod::Sase)` to opt in.

Compare both on synthetic graphs:
...
**Oversized splitting**: Communities larger than 25% of the graph (and at least 10 nodes) are recursively split by re-running the chosen partition method on their subgraph.
