...
| E5 | Matrix channel | P2 | 6-8 | clawft-channels/src/matrix/ | **Done (runtime ships, WEFT-159)** | Agent-06 / wave-0d | wave0d/weft-159-matrix-channel | MatrixChannelAdapter: real `/sync` long-poll with durable since-token, auto-join + invite accept, `m.room.message` → MessagePayload, `PUT .../send/m.room.message/{txn}` returns real event_id; wiremock integration tests |
