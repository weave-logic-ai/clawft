...
- **P1.6** — RESOLVED (WEFT-409, 2026-04-30; WEFT-563/WEFT-564 follow-up):
  `scripts/check-features.sh` was added 2026-02-25 and later **retired**
  (thin redirect shim → `scripts/build.sh gate`). `scripts/build.sh gate`
  is the canonical feature-flag validation entrypoint (full phase gate:
  native + WASI + browser + clippy + bundle-size + audit + more). SPARC
  BW5 + step0/BW1/master-plan/orchestrator docs point at `build.sh gate`;
  the old script name survives only as historical supersession notes.
